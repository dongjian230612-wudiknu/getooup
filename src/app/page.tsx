    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-50 bg-black text-white">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          {/* Desktop Navigation - 平铺显示 */}
          <nav className="hidden md:flex items-center gap-8">
            <a href="#standard" className="text-sm hover:text-gray-300 transition">Standard</a>
            <a href="#asiafit" className="text-sm hover:text-gray-300 transition">AsiaFit</a>
            <a href="#sunglasses" className="text-sm hover:text-gray-300 transition">Sunglasses</a>
            <a href="#lenses" className="text-sm hover:text-gray-300 transition">Lenses</a>
          </nav>
          
          <div className="flex items-center gap-4">
            <button className="p-2 hidden md:block"><User size={20} /></button>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div>
              ...
              <div className="font-medium">Better Price</div>
              <div className="text-sm text-gray-500">Total Value $49</div>
            </div>
            <div>
              <div className="text-3xl mb-2">🔍</div>
              <div className="font-medium">Lens</div>
              <div className="text-sm text-gray-500">China Factory</div>
            </div>
            <div>
              <div className="text-3xl mb-2">🚚</div>
              <div className="font-medium">Deliver Goods</div>
              <div className="text-sm text-gray-500">Door-To-Door</div>
            </div>
            <div>
              <div className="text-3xl mb-2">↩️</div>
              <div className="font-medium">30-Day</div>
              <div className="text-sm text-gray-500">Exchange & Return</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-medium mb-2">Browse best selling styles</h2>
            <p className="text-gray-500 text-sm">Statistics for the last month</p>
          </div>

          <div className="overflow-x-auto pb-4">
            <div className="flex gap-6" style={{ minWidth: 'max-content' }}>
              {products.map((product, idx) => (
                <div key={product.id} className="w-64 flex-shrink-0 group cursor-pointer" onClick={() => openProduct(product)}>
                    ...
                    <div className="aspect-[4/3] bg-gray-200 flex items-center justify-center overflow-hidden">
                      {product.image ? (
                        <img 
                          ...
                          onError={(e) => { (e.target as HTMLImageElement).src = ''; (e.target as HTMLImageElement).style.display = 'none'; e.currentTarget.parentElement!.innerHTML = '<span class="text-6xl">👓</span>'; }}
                        />
                      ) : (
                        <span className="text-6xl">👓</span>
                      )}
                    </div>
                    <button className="absolute bottom-3 right-3 bg-white/90 px-3 py-1.5 rounded-full text-xs flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                      <span>📷</span> AR Try-On
                    </button>
                    {idx < 6 && (
                      <span className="absolute top-3 left-3 bg-black text-white text-xs px-2 py-1">Best Seller</span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    {product.colors.slice(0, 4).map((color) => (
                      <div key={color} className="w-5 h-5 rounded-full border border-gray-300" style={{
                        backgroundColor: color === 'Black' ? '#000' : color === 'Tortoise' ? '#8B4513' : color === 'Clear' ? 'rgba(200,200,200,0.3)' : color === 'Grey' ? '#808080' : color === 'Bean Red' ? '#A0522D' : color === 'Purple' ? '#9370DB' : color === 'Tea Grey' ? '#9CA3AF' : color === 'Light Brown' ? '#D2B48C' : color === 'Brown' ? '#8B4513' : color === 'Gold' ? '#FFD700' : color === 'Silver' ? '#C0C0C0' : color === 'Clear Tea' ? '#D2B48C' : color === 'Black White' ? 'linear-gradient(45deg, #000 50%, #fff 50%)' : color === 'Clear Grey' ? 'rgba(128,128,128,0.3)' : color === 'Gunmetal' ? '#2C3539' : color === 'Pink' ? '#FFC0CB' : '#ccc'
                      }} title={color} />
                    ))}
                  </div>
                </div>
          ...
          <div className="text-center mt-8">
            <a href="#all" className="inline-block bg-black text-white px-8 py-3 rounded-full text-sm">View More</a>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl font-medium mb-2">Real People, Real Style</h2>
          <p className="text-gray-500 mb-8">Real wearers, captured in everyday moments</p>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-medium mb-2">Style & Vision</h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="bg-white rounded-lg overflow-hidden cursor-pointer hover:shadow-lg transition">
                <div className="aspect-video bg-gray-200" />
                <div className="p-4">
                  <h3 className="font-medium mb-2 text-sm">{post.title}</h3>
                  <p className="text-gray-500 text-xs line-clamp-2">{post.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 bg-white border-t">
        <div className="container mx-auto px-4 text-center max-w-md">
          <p className="text-gray-600 mb-4">Get the inside scoop on new frames, events, and more</p>
          <div className="flex gap-2">
            <input type="email" placeholder="Enter your email" className="flex-1 border rounded-full px-4 py-2 text-sm" />
            <button className="bg-black text-white px-6 py-2 rounded-full text-sm">Submit</button>
          </div>
        </div>
      </section>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {product.colors.map((color: string) => (
                <button key={color} onClick={() => setSelectedColor(color)} className={`p-3 border rounded-lg text-sm ${selectedColor === color ? 'border-black bg-gray-50' : ''}`}>
                  {color}
                </button>
              ))}
            </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3 mb-6">
              {lensOptions.map((lens: LensOption) => (
                <div key={lens.id} onClick={() => setSelectedLens(lens)} className={`p-4 border rounded-lg cursor-pointer ${selectedLens?.id === lens.id ? 'border-black' : ''}`}>
